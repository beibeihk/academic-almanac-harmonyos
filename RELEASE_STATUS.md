...
- 原生 1.2.0（1002000）：签名 APP `release-assets/packages/AcademicAlmanac-1.2.0-HarmonyOS5-signed.app` 已上传华为，已选作升级发布包；上架自检已启动。2026-09-21 提交审核，后台现显示“正在等待审核”。适用 HarmonyOS 5 及以上，不覆盖 HarmonyOS 4。
- Android 兼容应用：独立 APP ID `119074903`，包名 `com.beibeihk.academicalmanac.android`。签名 APK `release-assets/packages/AcademicAlmanac-1.2.0-HarmonyOS4-compatible-signed.apk` 已上传华为，已选作发布包；应用信息和版本信息已保存。2026-09-21 提交审核，后台现显示“正在审核”。该应用拟供仍支持 Android 应用的 HarmonyOS 4.x 设备下载，不能仅凭 Android 模拟器推定所有鸿蒙 4 真机兼容或保证应用市场审核通过。
